...
final class ChatStore: ObservableObject {
    ...
    nonisolated static func connectPromptText(
        roomName: String,
        ...
        if isEphemeral {
            return """
            You're being asked to collaborate with another AI agent over Cowchat in the Temporary room “\(roomName)”. Temporary-room messages are not persisted, so `history`, `--cursor-file`, and returning waits cannot recover a peer post sent while an agent is processing or replying. Do not start turn-based agent coordination in this room or claim that it is silence-safe. Tell me to choose or create a permanent room instead; once I provide it, \(connectionInstruction) and use that room's standard Cowchat connect prompt. A continuously connected external consumer may use this Temporary room only under explicit operator-specific delivery instructions. https://cowchat.cowboy.inc/skills.txt
            """
        }

        return """
        You're going to collaborate with another AI agent in real time over Cowchat. Read the Cowchat skill, \(connectionInstruction), and join the exact room \u{201C}\(roomName)\u{201D}. Choose one unique identity and pass the same stable `--name` and `--agent-id` on every command. Choose one `--cursor-file` path unique to this server, room, and identity; catch up existing messages once with `history --cursor-file` using that path. Then keep this task open and use exactly one returning `wait --loop --drain`; pass the same cursor file to every `send` and `wait`. After processing each wake and sending your reply, or after a timeout, immediately run that same wait again. Do not use `wait --follow` for agent delivery, and do not stop listening until a peer sends `conversation_end` or I explicitly tell you to stop. A returning wait works only while this task remains active: ordinary Cowchat messages cannot restart a completed Codex turn. If ended-turn follow-ups must wake Codex, require the operator to verify and run the configured `cowchat-codex relay` service before relying on that behavior. Start now without waiting for confirmation. https://cowchat.cowboy.inc/skills.txt
        """
    }
    ...
    func connectPrompt(for room: Room) -> String {
        Self.connectPromptText(
            roomName: room.name,
            connectionInstruction: agentConnectionInstruction,
            isEphemeral: room.ephemeral
        )
    }
